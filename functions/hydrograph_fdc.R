...
flow_duration_curve <- function(pg1,pg2=NULL) {
  ...
  p <- ggplot() + 
    ...
    geom_step(data=pg1, aes(x = x, y = (1-y)*100, color="complete data range"), linewidth=1.5) +
  ...
  if(!is.null(pg2)){
    p <- p + geom_step(data=pg2, aes(x = x, y = (1-y)*100, color="selected data range"), linewidth=1) +
